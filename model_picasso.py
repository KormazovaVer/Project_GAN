...
monet_images = PicassoDataset(root_picasso=ROOT_PICASSO,
...
gen_ptp = Generator().to(DEVICE)
gen_ptph = Generator().to(DEVICE)

opt_disc = optim.Adam(list(disc_p.parameters()) + list(disc_m.parameters()),
                      lr=LEARNING_RATE, betas=BETAS)
opt_gen = optim.Adam(list(gen_ptp.parameters()) + list(gen_ptph.parameters()),
                     lr=LEARNING_RATE, betas=BETAS)
...
def train_fn(gen_ptp, gen_ptph, disc_p, disc_m, loader, opt_gen, opt_disc, l1_loss, mse_loss, G_loss_list, D_loss_list):
    ...
    for idx, (picasso_img, photo_img) in enumerate(loop):
        ...
        fake_picasso = gen_ptp(photo_img)
        D_m_real = disc_m(picasso_img)
        D_m_fake = disc_m(fake_picasso.detach())
        ...
        # Cycle_loss
        cycle_picasso = gen_ptp(fake_photo)
        ...
        # Identity loss
        identity_m = gen_ptp(picasso_img)
